...
export function ripemd128(data) {
    var aa, bb, cc, dd, aaa, bbb, ccc, ddd, i, l, r, rr, t, tmp,
        hash = new Uint32Array([0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476]),
        bytes = data.length;

    const padding = Buffer.alloc(((bytes % 64) < 56 ? 56 : 120) - (bytes % 64));
    padding.writeUint8(0x80);

    // ending with check bits (= little endian 64-bit int, 8 * data.length)
    const ending = Buffer.allocUnsafe(8);
    bytes = bytes << 3;
    ending.writeUInt32LE(bytes, 0);
    ending.writeUint32LE(bytes >> 31 >> 1, 4);

    const full = Buffer.concat([data, padding, ending]);

    const x = new Uint32Array(full.buffer, full.byteOffset, full.byteLength / 4);
    // update hash
